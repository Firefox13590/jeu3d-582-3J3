...
public class TirScript_CreationBalle : MonoBehaviour
{
   ...
   #################################################*/
    ...
    void Tir()
    {
        ...
        GameObject instanceballe = Instantiate(balle, balle.transform.position, balle.transform.rotation);
        instanceballe.SetActive(true);
        instanceballe.GetComponent<Rigidbody>().linearVelocity = transform.forward * vitesseBalle;
    }
    //----------------------------------------------------------------------------------------------
