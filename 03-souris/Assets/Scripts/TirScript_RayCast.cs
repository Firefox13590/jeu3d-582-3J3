...
public class TirScript_RayCast : MonoBehaviour
{
   ...
   #################################################*/
    ...
    void Start()
    {
        ...
        boutDuFusil.GetComponent<LineRenderer>().material = new Material(Shader.Find("Sprites/Default"));
        boutDuFusil.GetComponent<LineRenderer>().startColor = Color.red;
        boutDuFusil.GetComponent<LineRenderer>().endColor = Color.yellow;
    }
    //----------------------------------------------------------------------------------------------
    ...
    */
    ...
    void Tir()
    {
        ...
        //RaycastHit hit;
        if(Physics.Raycast(boutDuFusil.transform.position, boutDuFusil.transform.forward, out RaycastHit hit, 50f))
        {
            GameObject instanceParticuleContact = Instantiate(impactTir, hit.point, Quaternion.identity);
            instanceParticuleContact.transform.localEulerAngles = Vector3.zero;
            Destroy(instanceParticuleContact, 3);
