...
public class BalleScript : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision infoCollisions)
    {
         ...
         */
        GameObject instanceParticuleContact = Instantiate(impactTir);
        instanceParticuleContact.transform.position = infoCollisions.GetContact(0).point;
        instanceParticuleContact.SetActive(true);
        instanceParticuleContact.transform.LookAt(personnage.transform);
        instanceParticuleContact.transform.Translate(0, .5f, 0);
        Destroy(instanceParticuleContact, 1);
        Destroy(gameObject);
    }
}
